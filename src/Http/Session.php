...
use Cawa\Http\SessionStorage\AbstractStorage;
...
class Session
{
    ...
    {
        // name
        $name = App::config()->getIfExists('session/name') ?? 'SID';
        $this->setName($name);

        // storage
        $class = App::config()->getIfExists('session/storage/class') ?? '\\Cawa\\Http\\SessionStorage\\FileStorage';
        $args = App::config()->getIfExists('session/storage/arguments') ?? [];
    ...
    public function init()
    {
        if (!self::$init) {
            ...
            if (!$this->id) {
                ...
                $maxDuration = App::config()->getIfExists('session/maxDuration') ?? ini_get('session.gc_maxlifetime');
    ...
    public function save() : bool
    {
        ...
        $ttl = App::config()->getIfExists('session/refreshTtl') ?? 60;
