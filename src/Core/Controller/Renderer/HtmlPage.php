...
class HtmlPage extends HtmlContainer
{
    ...
    {
        $sout = '<!DOCTYPE html lang="' . App::translator()->getLocale() . '">' . "\n";

        // default seo
        if (!$this->headTitle && $title = App::translator()->trans('seo.default/title')) {
            $this->setHeadTitle($title);
        }

        if (!$this->headDescription && $description = App::translator()->trans('seo.default/description')) {
        ...
        $language->addAttributes([
            ...
            'content' => App::translator()->getLocale()
        ]);
